...
const int window_height = desktopTemp.height - 150;
const int window_width = desktopTemp.width - 150;
...
// Checks if boids go out of the window and if so, wraps them around to the other side.
void Boid::bottomBorders()
{
	if (location.x < 0) applyForce(Pvector(0.2, 0));
	if (location.y < w_height / 2) applyForce(Pvector(0, 0.2));
	if (location.x > w_width) applyForce(Pvector(-0.2, 0));
	if (location.y > w_height) applyForce(Pvector(0, -0.2));
}
// Checks if boids go out of the window and if so, wraps them around to the other side.
void Boid::topBorders()
{
	if (location.x < 0) applyForce(Pvector(0.2, 0));
	if (location.y < 0) applyForce(Pvector(0, 0.2));
	if (location.x > w_width) applyForce(Pvector(-0.2, 0));
	if (location.y > w_height / 2) applyForce(Pvector(0, -0.2));
}
